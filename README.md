...
REST requests in this project can be separated to:
...
```
curl -X POST "domain:port/api/todo/tasks"
  -H "Content-Type: application/json"
  -H "accept: application/json"
  -d '{
    "title": "task title",
...
```
curl -X PUT "domain:port/api/todo/tasks{id}"
  -H "Content-Type: application/json"
  -H "accept: application/json"
  -d '{
...
```
curl -X DELETE "domain:port/api/todo/tasks{id}"
  -H "Content-Type: application/json"
  -H "accept: application/json"
  -H "Authorization: Bearer your_token"
```

### Reminder-related requests examples

Reminder can be created only in case if user already has at least 1 task.
Reminders are stored in Redis.
